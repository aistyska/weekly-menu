...
@section('content')
    ...
    <form method="post" action="/save-updated-recipe/{{ $recipe->id }}">
        ...
        <div class="custom-control custom-checkbox mb-2">
            <input type="checkbox" class="custom-control-input" id="needsSideDish" name="sideDish[]" value="needs_side_dish" @if(($recipe->needs_side_dish == 1 && old('_token') === null) || old('sideDish.0') == "needs_side_dish") checked @endif>
            <label class="custom-control-label" for="needsSideDish">Šiam patiekalui papildomai reikia garnyro</label>
        </div>
        <div class="custom-control custom-checkbox mb-2">
            <input type="checkbox" class="custom-control-input" id="isSideDish" name="sideDish[]" value="is_side_dish" @if(($recipe->is_side_dish == 1 && old('_token') === null) || old('sideDish.0') == "is_side_dish") checked @endif>
